...
class chess_game{
    ...
    chess_game(int gameID, bool newGame){
        ...
        //TODO: wczytywać grę z pliku o podanym gameid (tak samo zapisywać gry gdy przerwie się połączenie) 
        ...
        // TODO: Cały interfejs komunikacji server -- chessGame
        ...
        //         }
        //         MoveHis += f + " ";
        //         SaveGame(white,black,WhoMove,MoveHis);
        //     }
        // }
        // return 0;
    }
    ...
    int MoveFigureString(string inString){
        ...
        return MakeMove(white,black,&WhoMove,board,pos1,pos2,bw);
    }
...
void DrawBoard(figure **white, figure **black,figure* b[][8]){
    ...
    for(int i = 7;i>-1;i--){
        ...
        for(int k = 0;k<3;k++){
            ...
            if(k==0) cout<<"  ";
            for(int j = 0;j<8; j++){
                string tile2 = {tile[TileColor],tile[TileColor],tile[TileColor],tile[TileColor],tile[TileColor],tile[TileColor]};
                if(k==1&&b[j][i]!=nullptr){tile2[1] = ' '; tile2[2] = b[j][i]->getcolor(); tile2[3] = b[j][i]->getname(); tile2[4] = ' '; }
                cout<<tile2;
                TileColor = !TileColor;
            }
            if(k==1) cout<<" "<<i+1;
